...
export default function Quiz_Result() {
    ...
    function msg() {
        if (somaQuizz() >= 6) {
            return <p className={styles.text_result1}> <Image src={imagens.Warning} alt="Warning" width={20} height={20} className={styles.warning} /> Você provalvemente possui algum transtorno mental.</p>
        } else { return <p className={styles.text_result2}>Que tal uma relaxada!? Começe mudando sua rotina, exercícios físicos, boa alimentação e higiene do sono. Alterar alguns dos nossos maus habitos, muitas vezes já é o sufientes para uma melhora significativa no nosso estado mental! </p> }
    }
    ...
    function boxResut() {
        if (!getShow) {
            return <div className={styles.box3}>
                <h2 className={styles.text4_1}>Resultado</h2>
                <div className={styles.box_result}>
                    <div className={styles.quiz_box2}>
                        {msg()}
                    </div>
                    <form onSubmit={handleSubmit(onSubmit)} className={styles.msg}>
                        <div>
                        ...
                        <div>
                            <label htmlFor={styles.email}></label>
                            <input
                                type="email"
                                placeholder={'---Ex: email@gmail.com.br---'}
                                {...register('Email', { required: true })}
                                id="name_e-mail"
                                className={styles.name_e_mail} />
                        </div>
    ...
    function boxSendEmail() {
        if (getShow) {
            return <div className={styles.box3}>
                <div className={styles.box_send_email}>
    ...
    return (
        <>
            <article className={styles.art2_1}>
                {boxResut()}
                {boxSendEmail()}
            </article>
        </>
